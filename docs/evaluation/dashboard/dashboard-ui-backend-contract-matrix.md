...
| Demo PDF extract | PDF-Demo-Wissen | `DemoWorkspaceAgentBuilderCard` and PDF route caller | transient form/file state | multipart `file` only | `POST /api/sites/:siteId/conversation-engine/knowledge/pdf-extract` | `/admin/sites/:siteId/conversation-engine/knowledge/pdf-extract` | admin/operator scope or per-request customer workspace capability, PDF type/size boundary | in-memory extraction only, no persistence | no reload endpoint by design | not part of setup completion; test-only helper | admin/operator or individually assigned non-viewer customer; viewer/shared operator without scope forbidden | aligned and intentionally transient | PDF route, capability-auth, and customer workspace flow tests | aligned |
| Demo workspace config | assistantName, companyContext, assistantRole, targetAudience, tone, allowedTasks, blockedTasks, handoffAllowed, ticketAllowed, requiredFields | `DemoWorkspaceAgentBuilderCard.tsx` | local card form state | `demoWorkspace.*` allowlist only | `GET/PUT/DELETE /api/sites/:siteId/conversation-engine/demo-workspace/config` | `/admin/sites/:siteId/conversation-engine/demo-workspace/config` | admin/operator scope or per-request customer workspace capability plus backend config allowlist | persisted demo-workspace config only; no knowledge/chat/pdf state | same route GET | no setup completion; bounded internal workspace only | admin/operator or individually assigned non-viewer customer; viewer/shared operator without scope forbidden | aligned; persistent config is separate from transient knowledge/chat | Demo workspace card, config API, capability-auth, and customer workspace flow tests | aligned |
| Runtime pilot | interner Testchat / Runtime-Pilot | `LaunchStep` admin cards or bounded customer workspace and runtime pilot proxy | request-local test payload | `message`, `history`, `knowledgeSnippets`, `demoWorkspace`, `existingConversationState` | `POST /api/sites/:siteId/conversation-engine/runtime-pilot` | `/admin/sites/:siteId/conversation-engine/runtime-pilot` | admin/operator scope or per-request customer workspace capability and runtime pilot validation | no persistence by design | none | not a completion signal; internal evaluation surface with simulated handoff only | admin/operator or individually assigned non-viewer customer; viewer/shared operator without scope forbidden | bounded customer card excludes general diagnostics and live controls | runtime pilot route/API tests plus customer workspace flow | aligned_after_fix |
