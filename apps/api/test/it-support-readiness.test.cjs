...
test('it-support readiness reports blocking missing setup', () => {
  ...
  assert.ok(result.missing.some((entry) => /Ticket-Bestätigung/i.test(entry)));
  assert.equal(result.checks.requiredTicketFieldsValid, true);
});
...
test('it-support readiness reports warnings for optional operational gaps', () => {
  ...
  assert.ok(result.warnings.some((entry) => /ticket.created/i.test(entry)));
  assert.ok(result.warnings.some((entry) => /Wissensquellen/i.test(entry)));
