...
const CAPABILITY_KEY = 'knowledgeManagementV1';
const POWERUSER_ROLES = new Set(['owner', 'admin', 'manager', 'editor']);
...
export type CustomerKnowledgePoweruserAuthInput = {
  authorizationHeader?: unknown;
  dashboardTokenHeader?: unknown;
  targetSiteId?: unknown;
};

export type AuthorizedCustomerKnowledgeContext = {
  tenantId: string;
  siteId: string;
  actorId: string;
  actorRole: 'customer';
...
function parseCapability(value: unknown): string[] | null {
  if (!isRecord(value) || !hasExactKeys(value, new Set(['enabled', 'siteIds']))) return null;
  if (value.enabled !== true || !Array.isArray(value.siteIds) || value.siteIds.length === 0) return null;
  if (!value.siteIds.every(isCanonicalId) || new Set(value.siteIds).size !== value.siteIds.length) return null;
...
@Injectable()
export class CustomerKnowledgePoweruserAuthService {
  constructor(private readonly db: PrismaService) {}

  async authorize(input: CustomerKnowledgePoweruserAuthInput): Promise<AuthorizedCustomerKnowledgeContext> {
    ...
    try {
      ...
      const siteIds = parseCapability(principal.knowledge_capability);
      if (!siteIds) forbidden();

      const targetSiteId = isCanonicalId(input.targetSiteId) ? input.targetSiteId : null;
      if (!targetSiteId || !siteIds.includes(targetSiteId)) notFound();
  ...
  private verifyCredentials(input: CustomerKnowledgePoweruserAuthInput): VerifiedTenantSession {
  ...
  private async loadPrincipal(tenantUserId: string): Promise<PrincipalRow> {
    const result = await this.db.query<PrincipalRow>(
      `SELECT
         ...
         is_active,
         expires_at,
         metadata -> '${CAPABILITY_KEY}' AS knowledge_capability
       FROM tenant_users
       WHERE id = $1
       LIMIT 1`,
      [tenantUserId],
  ...
  private assertPrincipal(session: VerifiedTenantSession, principal: PrincipalRow): void {
    ...
    if (
      ...
      || !POWERUSER_ROLES.has(principal.role)
