...
export async function GET(
  _req: Request,
  context: { params: Promise<{ siteId: string; ticketId: string }> },
) {
  const auth = await requireSession({ allowCustomer: true });
  if (auth.response) return auth.response;

  const { siteId, ticketId } = await context.params;
  try {
  ...
  const response = await fetchDashboardBackend(
    `/admin/sites/${encodeURIComponent(siteId)}/it-support/tickets/${encodeURIComponent(ticketId)}`,
    {
      method: "GET",
